...
?>
<div class="wrap">
<h1><?php _e( 'Delete Comments', 'easy-disable-comments') ?></h1>
<?php

global $wpdb;
$comments_count = $wpdb->get_var("SELECT count(comment_id) from $wpdb->comments");
if ( $comments_count <= 0 ) { ?>
<p><strong><?php _e( 'No comments available for deletion.', 'easy-disable-comments') ?></strong></p>
</div>
<?php
	return;
}
	...
	if( $_POST['delete_mode'] == 'delete_everywhere' ) {
		if ( $wpdb->query( "TRUNCATE $wpdb->commentmeta" ) != FALSE ){
            if ( $wpdb->query( "TRUNCATE $wpdb->comments" ) != FALSE ){
            	...
            	echo "<p style='color:green'><strong>" . __( 'All comments have been deleted.', 'easy-disable-comments' ) . "</strong></p>";
            } else {
	            echo "<p style='color:red'><strong>" . __( 'Internal error occured. Please try again later.', 'easy-disable-comments' ) . "</strong></p>";
            }
        } else {
            echo "<p style='color:red'><strong>" . __( 'Internal error occured. Please try again later.', 'easy-disable-comments' ) . "</strong></p>";
        }
		...
		if ( ! empty( $delete_post_types ) ) {
			...
			foreach ( $delete_post_types as $delete_post_type ) {
				...
				echo "<p style='color:green'><strong>" . sprintf( __( 'All comments have been deleted for %s.', 'easy-disable-comments' ), $post_type_label ) . "</strong></p>";
			}

            $wpdb->query( "OPTIMIZE TABLE $wpdb->commentmeta" );
            $wpdb->query( "OPTIMIZE TABLE $wpdb->comments" );

            echo "<h4 style='color:green'><strong>" . __( 'Comment Deletion Complete', 'easy-disable-comments') . "</strong></h4>";
		}
...
<ul>
<li><label for="delete_everywhere"><input type="radio" id="delete_everywhere" name="delete_mode" value="delete_everywhere" <?php checked( $this->options['remove_everywhere'] );?> /> <strong><?php _e( 'Everywhere', 'easy-disable-comments') ?></strong>: <?php _e( 'Delete all comments in WordPress.', 'easy-disable-comments') ?></label>
	<p class="indent"><?php printf( __( '%s: This function and will affect your entire site. Use it only if you want to delete comments <em>everywhere</em>.', 'easy-disable-comments' ), '<strong style="color: #900">' . __('Warning', 'easy-disable-comments') . '</strong>' ); ?></p>
</li>
<li><label for="selected_delete_types"><input type="radio" id="selected_delete_types" name="delete_mode" value="selected_delete_types" <?php checked( ! $this->options['remove_everywhere'] );?> /> <strong><?php _e( 'For certain post types', 'easy-disable-comments') ?></strong>:</label>
	<p></p>
	...
	<p class="indent" id="extradeletetypes"><?php _e( 'Only the built-in post types appear above. If you want to disable comments on other custom post types on the entire network, you can supply a comma-separated list of post types below (use the slug that identifies the post type).', 'easy-disable-comments' ); ?>
	<br /><label><?php _e( 'Custom post types:', 'easy-disable-comments' ); ?> <input type="text" name="delete_extra_post_types" size="30" value="<?php echo implode( ', ', (array) $this->options['extra_post_types'] ); ?>" /></label></p>
	<?php endif; ?>
	<p class="indent"><?php printf( __( '%s: Deleting comments will remove existing comment entries in the database and cannot be reverted without a database backup.', 'easy-disable-comments'), '<strong style="color: #900">' . __('Warning', 'easy-disable-comments') . '</strong>' ); ?></p>
</li>
</ul>

<?php wp_nonce_field( 'delete-comments-admin' ); ?>
<h4><?php _e( 'Total Comments:', 'easy-disable-comments' ); ?> <?php echo $comments_count; ?></h4>
<p class="submit"><input class="button-primary" type="submit" name="delete" value="<?php _e( 'Delete Comments', 'easy-disable-comments') ?>"></p>
</form>
</div>
<script>
